  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap print:hidden">
        <div>
          <h1 className="text-3xl font-black text-[#3a2c1e]">Informes</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="bg-surface border border-[#e4d8c4] text-[#3a2c1e] text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand"
          >
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 bg-brand hover:bg-brand-light text-[#3a2c1e] text-sm font-bold px-4 py-2 rounded-lg transition"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <SummaryCard label="Atletas" value={totalAthletes} color="text-[#3a2c1e]" />
        <SummaryCard label="En Riesgo ACWR" value={atRisk.length} color="text-[#b5761f]" />
        <SummaryCard label="Alertas Rojas" value={redAlerts.length} color="text-[#c0492f]" />
        <SummaryCard
      ...
      {teams.map((team) => (
        <div key={team.id} className="space-y-4 print:break-before-page print:pt-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-[#3a2c1e]">{team.name}</h2>
          ...
          {/* Athletes table */}
          <div className="bg-surface border border-[#e4d8c4] rounded-xl overflow-hidden print:border-gray-300 print:rounded-none">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#e4d8c4] print:border-gray-300">
                  <th className="text-left px-4 py-3 text-[#8a7660] font-semibold print:text-gray-500">Atleta</th>
                  <th className="text-center px-3 py-3 text-[#8a7660] font-semibold print:text-gray-500">ACWR</th>
                  <th className="text-center px-3 py-3 text-[#8a7660] font-semibold print:text-gray-500">Zona</th>
                  <th className="text-center px-3 py-3 text-[#8a7660] font-semibold print:text-gray-500">EVA</th>
                  <th className="text-center px-3 py-3 text-[#8a7660] font-semibold print:text-gray-500">TMD</th>
                  <th className="text-center px-3 py-3 text-[#8a7660] font-semibold print:text-gray-500">H/Q</th>
                  <th className="text-center px-3 py-3 text-[#8a7660] font-semibold print:text-gray-500">FMS</th>
                  <th className="text-center px-3 py-3 text-[#8a7660] font-semibold print:text-gray-500">Alertas</th>
                </tr>
              </thead>
              <tbody>
                {team.athletes.map((athlete) => {
                  ...
                  return (
                    <tr
                      key={athlete.id}
                      className="border-b border-[#e4d8c4] print:border-gray-200"
                    >
                      <td className="px-4 py-3">
                        <p className="text-[#3a2c1e] font-semibold print:text-gray-900">{athlete.full_name}</p>
                        <p className="text-[#8a7660] text-xs">{athlete.email}</p>
                      </td>
                      ...
                      <td className="px-3 py-3 text-center">
                        {athlete.latest_hq ? (
                          <span className={`text-xs font-bold ${
                            athlete.latest_hq.risk_flag ? "text-[#c0492f]" : "text-[#6f9c4a]"
                          }`}>
                      ...
                      <td className="px-3 py-3 text-center">
                        {athlete.latest_biomech ? (
                          <span className={`text-xs font-bold ${
                            athlete.latest_biomech.fms_injury_risk ? "text-[#d9702a]" : "text-[#6f9c4a]"
                          }`}>
                      ...
                      <td className="px-3 py-3 text-center">
                        {athlete.prescriptions.length > 0 ? (
                          ...
                          <span className="text-[#6f9c4a] text-xs">✓</span>
                        )}
                      </td>
                    </tr>
                  );
      ...
      {/* Disclaimer */}
      <p className="text-[#b0a08c] text-xs print:text-gray-400">
...
function SummaryCard({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="bg-surface rounded-xl p-5 border border-[#e4d8c4] print:border-gray-200">
      <p className={`text-3xl font-black ${color}`}>{value}</p>
      <p className="text-[#8a7660] text-sm mt-1 print:text-gray-500">{label}</p>
    </div>
  );
}
